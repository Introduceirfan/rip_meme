...
<div class="max-w-xl mx-auto">
    ...
    <form action="/memes/{{ $meme->id }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="date" name="died_at" value="{{ $meme->died_at }}" class="w-full bg-gray-700 border border-gray-400 rounded px-4 py-2 text-white">
        </div>
        
        <div>
            <label class="block text-sm text-gray-400 mb-1">Mati gara-gara</label>
            <input type="text" name="cause_of_death" value="{{ $meme->cause_of_death }}" class="w-full bg-gray-700 border border-gray-400 rounded px-4 py-2 text-white">
        </div>
